...
Decision은 새 환경변수나 대체 feature flag 없이 `segment_ids` 신규 run과 failed-only 자동 next-loop를 항상 처리합니다. 배포 전 segment scope expand·backfill·finalize와 Dashboard scope 응답 파싱을 완료하고, 검증된 Decision image/revision으로 기능을 활성화합니다.

Finalize 전 배포하면 기존 `uq_promotion_runs_loop` 때문에 같은 promotion/loop의 다른 scope가 충돌할 수 있습니다. 기존 run의 scope·fingerprint·target/fallback experiment·lineage 무결성 실패로 발생하는 409는 정상 검증입니다.
...
5. dev ECS service의 모든 task를 같은 검증 완료 Decision image/revision으로 교체하고 smoke test, Dashboard/Advertisement 계약, dispatch 재시도 확인
6. 운영 ECS service의 모든 task를 같은 검증 완료 Decision image/revision으로 교체하고 문제 시 이전 검증 완료 revision으로 service 전체 롤백
...
Dashboard rollout blocker는 별도로 해소해야 합니다. `promotion_id + loop_count + LIMIT 1` 조회는 금지하고 단건 run은 `promotion_run_id`, next-loop는 `parent_ad_experiment_id` 또는 preparation/activation lineage로 추적합니다.
