...
|---|---:|---|---|---|
| 제주·오키나와 여름 예약 의도 검증 | 1차 | 목표 미달 | 220 → 172 → 130 → 58 → 7명 | 관심 고객군 안의 현재 예약 의도 차이 추가 확인 |
| 제주·오키나와 여름 예약 의도 검증 | 2차 | 목표 미달 | 216 → 180 → 144 → 64 → 11명 | 반복 조회 고객 12.3%, 그 외 고객 2.0%와 숙박 총액 차이를 관측 |
...
제주·오키나와 fixture는 평가 시점의 반복 조회 코호트와 현재 30일 행동 데이터를 함께 생성합니다. 고객군 후보 도우미에서는 `20~30대` 620명, 목적지 숙소 상세 2회 이상 220명, 예약 시작 후 미완료까지 모두 만족하는 고객 132명을 순차적으로 조회할 수 있습니다.
